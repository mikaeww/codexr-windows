...
func Start(accountID, codexHome, executable string, args, baseEnv []string, inbound chan<- Inbound, diagnostics io.Writer) (*Child, error) {
	if diagnostics == nil {
...
// Done closes once the child process has exited, so a caller that is about to
// replace its own process image can make sure it leaves no orphaned app-server
// behind.
...
func (c *Child) readLoop(stdout io.Reader) {
	...
	for scanner.Scan() {
		...
		if err != nil {
			fmt.Fprintf(c.diagnostics, "codex-mux: %s emitted invalid JSON: %v\n", c.accountID, err)
			continue
		}
	...
	if err := scanner.Err(); err != nil {
		fmt.Fprintf(c.diagnostics, "codex-mux: read %s app-server: %v\n", c.accountID, err)
	}
}
