...
import (
	...
	"github.com/mikaeww/codexr-windows/internal/backend"
...
type Options struct {
	...
	Store          *state.Store
	Output         io.Writer
	// Diagnostics receives child app-server logs and the multiplexer's own
	// warnings. It must not be the terminal while a full-screen UI owns it.
...
func (m *Multiplexer) Start(ctx context.Context) error {
	for _, account := range m.store.Accounts() {
		if _, err := m.startChild(ctx, account); err != nil {
			fmt.Fprintf(m.diagnostics, "codex-mux: start account %s: %v\n", account.ID, err)
		}
	}
	...
	for {
		...
		case <-ticker.C:
			if err := m.store.SyncManagedConfig(); err != nil {
				fmt.Fprintf(m.diagnostics, "codex-mux: sync shared plugin config: %v\n", err)
			}
		}
	}
}
...
func (m *Multiplexer) write(message protocol.Message) {
	...
	if err != nil {
		fmt.Fprintf(m.diagnostics, "codex-mux: encode response: %v\n", err)
		return
	}
	m.writeRaw(encoded)
}
